  ...
  return (
    <>
      ...
      <aside className="personal-panel" aria-label="Watchlist and research journal">
        <header>
          <div className="personal-title"><BookOpen size={18} /><div><strong>My research</strong><span>Saved on this device</span></div></div>
          <button className="panel-close" onClick={onClose} aria-label="Close"><X size={19} /></button>
        </header>

        <section className="personal-section">
          <div className="section-heading"><strong>Watchlist</strong><span>{research.watchlist.length} saved</span></div>
          <form className="ticker-form" onSubmit={addTicker}>
            <input value={symbol} onChange={(event) => setSymbol(event.target.value)} placeholder="AAPL or ^GSPC" aria-label="Ticker symbol" />
            <button type="submit"><Plus size={15} /> Add</button>
          </form>
          <div className="watchlist-items">
            {research.watchlist.map((item) => (
              <div className={`watchlist-item ${selectedSymbol === item.symbol ? 'selected' : ''}`} key={item.symbol}>
                <button className="watchlist-select" onClick={() => setSelectedSymbol(item.symbol)}>{item.symbol}</button>
                <button
                  className="research-ticker"
                  onClick={() => {
                    onResearch(`Give me a current overview and price-risk analysis for ${item.symbol}.`)
                    onClose()
                  }}
                  aria-label={`Research ${item.symbol}`}
                ><ArrowUpRight size={14} /></button>
                <button className="delete-item" onClick={() => research.removeSymbol(item.symbol)} aria-label={`Remove ${item.symbol}`}><Trash2 size={13} /></button>
              </div>
            ))}
            {!research.watchlist.length && <p className="personal-empty">Add the stocks and indices you regularly follow.</p>}
          </div>
        </section>

        <section className="personal-section journal-section">
          <div className="section-heading"><strong>Research journal</strong><span>{selectedSymbol || 'No ticker selected'}</span></div>
          <form className="journal-form" onSubmit={saveNote}>
            ...
            />
            <button type="submit" disabled={!selectedSymbol || !note.trim()}>Save note</button>
          </form>
          {error && <p className="personal-error">{error}</p>}
          <div className="journal-entries">
            {visibleEntries.map((entry) => (
              <article key={entry.id}>
                <header><strong>{entry.symbol}</strong><button onClick={() => research.removeJournalEntry(entry.id)} aria-label="Delete note"><Trash2 size={12} /></button></header>
                <p>{entry.note}</p>
                <time>{entryDate(entry.createdAt)}</time>
              </article>
            ))}
            {!visibleEntries.length && selectedSymbol && <p className="personal-empty">No notes for {selectedSymbol} yet.</p>}
          </div>
        </section>
      </aside>
    </>
